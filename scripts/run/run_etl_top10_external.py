#!/usr/bin/env python3
"""
Orquestra consultas em bases externas a partir do resultado Top 10.

Fluxo:
1. Lê o arquivo do Top 10 (parquet/csv/xlsx/txt).
...
DEFAULT_TOP10 = STAGING_DIR / "top10_candidates.parquet"
...
def _resolve_top_input(arg_value: str | None) -> Path:
    if arg_value:
        return Path(arg_value)
    return DEFAULT_TOP10


def _load_top10_dataframe(file_path: Path) -> pd.DataFrame:
    if not file_path.exists():
        raise FileNotFoundError(f"Arquivo Top 10 não encontrado: {file_path}")
    ...
    raise ValueError(f"Formato não suportado para Top 10: {suffix}")


def _normalize_top10_for_api(df: pd.DataFrame) -> pd.DataFrame:
    source = df.copy()
...
def _write_inputs(df_top10: pd.DataFrame) -> tuple[Path, Path]:
    STAGING_DIR.mkdir(parents=True, exist_ok=True)

    api_input = STAGING_DIR / "top10_external_input.csv"
    chemspider_input = STAGING_DIR / "top10_chemspider_input.txt"

    df_api = _normalize_top10_for_api(df_top10)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Integra ETLs externos usando o resultado Top 10 como entrada."
    )
    parser.add_argument(
        "--top10",
        default=None,
        help="Arquivo do ranking Top 10 (parquet/csv/xlsx/txt).",
    )
    parser.add_argument(
        "--sources",
        nargs="+",
...
def main() -> None:
    args = parse_args()
    top10_path = _resolve_top_input(args.top10)

    print("\n=== Integração de Bases Externas via Top 10 ===")
    print(f"Top 10 de entrada: {top10_path}")
    print(f"Fontes selecionadas: {', '.join(args.sources)}")

    df_top10 = _load_top10_dataframe(top10_path)
    if df_top10.empty:
        raise ValueError("Arquivo Top 10 está vazio.")

    api_input, chemspider_input = _write_inputs(df_top10)
